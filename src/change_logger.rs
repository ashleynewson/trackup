...
impl<'c, 'd> ChangeLogger<'c, 'd> {
    pub fn new(config: &'c Config, devices: &'d Vec<Device>) -> Self {
        Self {
            config,
            devices,
        }
    ...
    pub fn run(&self, log_channel: Sender<(usize, usize)>, sync_barrier_channel: Receiver<Arc<Barrier>>) {
        ...
        let _current_tracer_setup = DoUndo::new(
            ...
            || {append_to_file_at_path(&self.config.tracing_path.join("current_tracer"), &old_current_tracer).warn_if_err();},
        );
        ...
        let _tracer_option_bin_setup = DoUndo::new(
            ...
            || {append_to_file_at_path(&self.config.tracing_path.join("options/bin"), &old_tracer_option_bin).warn_if_err();},
        );
        ...
        let _tracer_option_context = DoUndo::new(
            ...
            || {append_to_file_at_path(&self.config.tracing_path.join("options/context-info"), &old_tracer_option_context).warn_if_err();},
        );
        ...
        let _buffer_size = DoUndo::new(
            ...
            || {append_to_file_at_path(&self.config.tracing_path.join("buffer_size_kb"), &old_buffer_size).warn_if_err();},
        );
