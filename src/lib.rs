...
pub fn backup_device(config: &config::Config) -> Result<(),()> {
    ...
    let mut backup_file =
        if config.reuse_output {
            backup_file::BackupFile::use_file(config.destination, device_file.get_size()).expect("Could not open backup file (reuse)")
        } else {
            backup_file::BackupFile::create_file(config.destination, device_file.get_size()).expect("Could not open backup file (create)")
        };
